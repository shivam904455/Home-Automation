...
func (store PgressStore) GetActuators() (*[]model.Actuator, error) {
	...
	if resp.Error != nil {
		util.Log(model.LogLevelError, model.PgressPackageLevel, model.GetActuators,
...
func (store PgressStore) GetActuatorsByFilter(filter map[string]string) (*[]model.Actuator, error) {
	...
	if err != nil {
		util.Log(model.LogLevelError, model.PgressPackageLevel, model.GetActuatorByFilter,
			"error while reading actuator data", err)
...
func (store PgressStore) GetActuator(actuatorID string) (*model.Actuator, error) {
	...
	if resp.Error != nil {
		util.Log(model.LogLevelError, model.PgressPackageLevel, model.GetActuator,
...
func (store PgressStore) DeleteActuator(actuatorID string) error{
	...
	var actuator model.Actuator
	util.Log(model.LogLevelInfo, model.PgressPackageLevel, model.DeleteActuator, "deleting actuator data", actuatorID)
	if err := store.DB.First(&actuator, `"id" = '`+actuatorID+`'`).Error; err != nil {
		util.Log(model.LogLevelError, model.PgressPackageLevel, model.DeleteActuator,
			"error while deleting actuator data", err)
		return fmt.Errorf("actuator not found for given id, ID = %v", actuatorID)
	}

...
func (store PgressStore) GetPointRates() (*[]model.PointRate, error) {
	...
	if resp.Error != nil {
		...
		return &pointRates, fmt.Errorf("error while fetching pointRate record from DB, err = %v", resp.Error)
	}
...
func (store PgressStore) GetPointRatesByFilter(filter map[string]string) (*[]model.PointRate, error) {
	...
	util.Log(model.LogLevelInfo, model.PgressPackageLevel, model.GetPointRateByFilter,
		"reading pointRate data from db based on filter", filter)
	var query *gorm.DB
...
func (store PgressStore) GetPointRate(pointRateID string) (*model.PointRate, error) {
	...
	if resp.Error != nil {
		...
		return &pointRate, fmt.Errorf("error while fetching pointRate record from DB for given id, err = %v", resp.Error)
	}
...
func (store PgressStore) UpdatePointRate(pointRate *model.PointRate) error {
	...
	if resp.Error != nil {
		util.Log(model.LogLevelError, model.PgressPackageLevel, model.UpdatePointRate,
...
func (store PgressStore) DeletePointRate(pointRateID string) error{
	...
	if err := store.DB.First(&pointRate, `"id" = '`+pointRateID+`'`).Error; err != nil {
		util.Log(model.LogLevelError, model.PgressPackageLevel, model.DeletePointRate,
			"error while deleting pointRate data", err)
		return fmt.Errorf("pointRate not found for given id, ID = %v", pointRateID)
	}
	resp := store.DB.Delete(pointRate)
	if resp.Error != nil {
